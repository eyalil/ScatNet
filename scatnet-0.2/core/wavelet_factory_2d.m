...
function [Wop, filters] = wavelet_factory_2d(size_in, filt_opt, scat_opt)
    ...
    switch filt_opt.filter_type
        ...
        %Eyal: added more filter types for testing purposes
        case 'EyalRandom'
            filt_opt = rmfield(filt_opt, 'filter_type');
            filters = random_filter_bank_2d(size_in, filt_opt);
            
        %Ordered random (sampled in some ordered manner)
        case 'OrderedRandom'
            filt_opt = rmfield(filt_opt, 'filter_type');
            filters = random_ordered_filter_bank_2d(size_in, filt_opt);           
            
        %Uniform random (sample uniformly from a circle in the frequency
        %domain)
        case 'UniformRandom'
            filt_opt = rmfield(filt_opt, 'filter_type');
